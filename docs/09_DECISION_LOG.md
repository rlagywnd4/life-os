...
## 2026-08-03 - 프로젝트 진행률은 말단 활동을 우선 계산하고 다음 행동은 명시적으로 지정한다
...
- 프로젝트 단계와 부모 활동까지 단순 합산하면 하나의 실행 결과가 여러 번 진행률에 반영된다.
- 시스템이 다음 행동을 자동으로 바꾸면 사용자가 의도적으로 고른 우선순위를 덮어쓸 수 있다.

Decision:

- 완료된 프로젝트는 100%로 표시하고, 그 외에는 완료된 말단 활동 비율을 진행률로 사용한다. 실행 활동이 전혀 없을 때만 마일스톤 완료율을 사용한다.
- `projects.next_action_id`는 같은 사용자·프로젝트의 활동만 참조하도록 DB trigger로 검증한다. 값이 없을 때는 UI가 미완료 말단 활동을 추천하지만 자동 저장하지 않는다.

Rationale:

- 말단 활동은 실제 실행 단위이므로 중복 없이 진행 상태를 설명한다.
- 추천과 명시적 선택을 분리하면 사용자가 계획의 통제권을 유지한다.

Alternatives:

- 부모와 자식을 모두 합산하는 방식은 깊이가 늘수록 진행률을 왜곡하므로 제외했다.
- 다음 행동을 완료 시 자동 변경하는 방식은 사용자의 맥락·우선순위를 무시할 수 있어 제외했다.

Impact:

- 프로젝트 목록과 상세는 같은 공통 도메인 함수를 사용한다.
- 단계는 별도 테이블이 아니라 `action_items.is_stage`인 최상위 활동으로 저장한다.

PRD Impact:

- 없음. PRD의 프로젝트→다음 행동 원칙을 현재 데이터 구조에 맞게 구체화한 구현 결정이다.
