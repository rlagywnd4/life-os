...
| Deployment Automation                     | Partial | main에서 테스트, 앱 빌드, Supabase migration을 자동화함. GitHub 비밀값 등록과 최초 원격 스키마 확인을 완료했고, 검증된 기존 2개 migration의 1회 history 복구 및 운영 배포 재검증이 필요함. |
| Codex Delivery Process                    | Done    | 기능 PR은 사용자가 Preview 또는 테스트 빌드로 확인한 뒤 병합하며, 영문 Conventional Commit·한국어 PR·Squash merge·안전한 복구 규칙을 적용함.                                               |
