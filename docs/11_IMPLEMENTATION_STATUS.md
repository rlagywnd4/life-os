...
| Native App V1                             | Partial | iPhone·Mac 공용 SwiftUI 기반과 Supabase 로그인, 선택적인 개인 계정 원클릭 로그인, 로그인 실패 원인 안내, `UNREVIEWED` Inbox 읽기 흐름을 구현함. 실제 계정 로그인 확인, Inbox 추가·수정·삭제, 검색, 실시간 갱신은 아직 남아 있음. |
