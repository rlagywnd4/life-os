...
| Calendar                                  | Partial | 네이티브에서 월간·주간 보기, 활동 실행일·시간·마감일, 일반 일정 CRUD, 완료·드래그/날짜 재배치, Today 시간표와 계획량 안내를 구현함. 운영 migration 적용과 실제 iPhone·Mac 조작 검증, 웹 달력은 남아 있음.    |
...
1. 달력 migration을 운영 Supabase에 적용하고 실제 계정으로 일정·활동 재배치를 확인한다.
2. iPhone과 Mac에서 월간·주간 달력의 드래그, 일정 편집, Today 반영을 직접 확인한다.
3. 검증 후 같은 실행일 기준을 사용하는 웹 달력을 구현한다.
